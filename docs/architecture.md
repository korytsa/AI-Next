...
│   │   └── rag/               # RAG endpoint (mock documents)
│   ├── chat/                  # Страница чата
│   │   ├── components/        # Компоненты чата
│   │   │   └── settings/     # Компоненты табов настроек
│   │   ├── hooks/             # Custom hooks
...
│   │   └── rag-data.ts        # Mock данные для RAG
...
- **ChatHeader** - Шапка с кнопкой настроек и управлением
- **MessagesList** - Список сообщений
- **MessageBubble** - Отдельное сообщение
- **ChatInput** - Поле ввода с кнопками (включая сохранение шаблона)
- **ChatSettingsDrawer** - Боковая панель настроек с табами
- **PromptTemplates** - Шаблоны промптов
- **LoadingIndicator** - Индикатор загрузки
- **UserNameBadge** - Бейдж с именем пользователя

#### Settings Components (`app/chat/components/settings/`)
- **AISettingsTab** - Настройки AI (модель, режим, CoT, streaming, RAG)
- **VoiceSettingsTab** - Настройки голосового вывода
- **ExportTab** - Экспорт истории чата
- **MetricsTab** - Метрики использования API
- **ErrorsTab** - Список ошибок приложения
...
### `/api/rag` (GET)
Получение mock документов для RAG (Retrieval Augmented Generation). Возвращает список документов из `rag-data.ts`.
